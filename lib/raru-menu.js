  ...
  constructor(elm, options) {
    ...
    elm.addEventListener('pointerup', (evt) => {
      if (evt.button === 2) {
        ...
        contextMenu.querySelector('.context-menu-frame').style.height = "auto";
        ...
        window.requestAnimationFrame(() => {
          ...
          if (evt.clientX + contextMenu.offsetWidth > htmlElm.clientWidth) {
            const left = Math.max(htmlElm.clientWidth - contextMenu.offsetWidth, 0);
            contextMenu.style.marginLeft = `${left}px`;
          }
          if (evt.clientY + contextMenu.offsetHeight > htmlElm.clientHeight) {
            const top = Math.max(htmlElm.clientHeight - contextMenu.offsetHeight, 0);
            contextMenu.style.marginTop = `${top}px`;
          }
          const frame = contextMenu.querySelector('.context-menu-frame')
          console.debug(frame.clientHeight, htmlElm.clientHeight);
          if (frame.clientHeight >= htmlElm.clientHeight) {
            frame.style.height = `${htmlElm.clientHeight}px`;
          }
